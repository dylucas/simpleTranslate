...
| 令牌 | 暗色 | 浅色 | 用途 |
| --- | --- | --- | --- |
| `--primary` | `#a5a0ff` | `#5146d8` | 主交互色、聚焦环、激活态 |
| `--primary-hover` | `#b7b3ff` | `#4338ca` | 悬停反馈 |
| `--primary-active` | `#8d86f7` | `#3730a3` | 按下态 |
| `--primary-soft` | `rgba(139,130,246,.14)` | `rgba(79,70,229,.09)` | 选中/激活底色 |
| `--accent-glow` | `rgba(124,112,246,.24)` | `rgba(79,70,229,.16)` | 主色光晕阴影 |
| `--accent-grad` | `linear-gradient(135deg,#7067ed,#8b5cf6)` | `linear-gradient(135deg,#5b52df,#7c3aed)` | 主按钮/品牌徽标渐变 |
...
| `--danger-strong` | `#991b1b` | `#991b1b` | 错误 Toast 高对比背景 |
...
| 令牌 | 暗色 | 浅色 | 用途 |
| --- | --- | --- | --- |
| `--bg-base` | `#0c0e13` | `#f4f5f9` | 画布最底层 |
| `--bg-sidebar` | `#101219` | `#fbfbfd` | 侧边栏、状态栏 |
| `--bg-surface` | `#161922` | `#ffffff` | 卡片表面 |
| `--bg-elevated` | `#1b1f2a` | `#ffffff` | 弹窗、下拉选项 |
| `--bg-input` | `#11141c` | `#f7f7fa` | 输入框、分组控件 |
| `--bg-hover` | `#222735` | `#f0f1f6` | 悬停反馈 |
| `--bg-overlay` | `rgba(5,7,12,.68)` | `rgba(15,23,42,.35)` | 模态遮罩 |
| `--bg-workspace` | 紫色环境光渐变 | 紫色环境光渐变 | 主工作区背景 |
| `--bg-panel` | 半透明 surface | 半透明白色 | 输入面板、顶栏 |
| `--bg-panel-muted` | 半透明深色 | 半透明浅灰 | 译文面板 |

**文本：**

| 令牌 | 暗色 | 浅色 | 用途 |
| --- | --- | --- | --- |
| `--text-main` | `#f1f2f6` | `#171927` | 主要文本 |
| `--text-sec` | `#aab1c0` | `#596174` | 次要文本、标签 |
| `--text-muted` | `#858da0` | `#687185` | 弱化文本、快捷键提示 |
...
| 令牌 | 暗色 | 浅色 | 用途 |
| --- | --- | --- | --- |
| `--border` | `#2a2f3e` | `#e4e8f0` | 默认分隔线、卡片边 |
| `--border-soft` | `#202431` | `#eef1f7` | 轻量分隔 |
| `--border-strong` | `#3a4154` | `#d1d8e4` | 滚动条、强分隔 |
...
| 断点 | 行为 |
| --- | --- |
| `> 980px` | 默认布局：侧边栏 208px、模式按钮显示图标与文字、双栏编辑器 |
| `721–980px` | 侧边栏收紧至 184px；模式按钮只显示图标；编辑器间距收紧 |
| `≤ 720px` | 侧边栏固定为 68px 图标栏；头部换行；编辑器上下堆叠；状态栏隐藏快捷键提示 |

侧边栏可折叠（208px ↔ 68px），折叠时导航文字与引擎选择隐藏、工具按钮纵向居中排列。
